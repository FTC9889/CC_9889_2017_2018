...
public class RED_FOWARD {
    public RED_FOWARD(AutoModeBase M, RelicRecoveryVuMark column){
        ...
        switch (column){
            case LEFT:
                left = M.Robot.getDrive().getLeftTicks() + inches2Ticks(8);
                right = M.Robot.getDrive().getRightTicks() + inches2Ticks(8);
            ...
            case CENTER:
                ...
                while (turning && M.opModeIsActive()) {
                    M.updateTelemetry();
                    if (M.Robot.getDrive().getGyroAngleDegrees() < -106)
                        turning = false;
                    M.idle();
                }
                ...
                while(M.opModeIsActive()){
                    ...
                    else if(M.Robot.getDrive().getGyroAngleDegrees() < -106)
